------------------ change this -------------------
admins = {'steam:110000105959047' -- 'license:1234975143578921327',
}
...
-------------------- DON'T CHANGE THIS --------------------
AvailableWeatherTypes = {'EXTRASUNNY', 'CLEAR', 'NEUTRAL', 'SMOG', 'FOGGY', 'OVERCAST', 'CLOUDS', 'CLEARING', 'RAIN',
  ...
  if source ~= 0 then
    if isAllowedToChange(source) then
      ...
      TriggerClientEvent('chatMessage', source, '', {255, 255, 255},
        '^8Error: ^1You are not allowed to use this command.')
    end
  else
    freezeTime = not freezeTime
    if freezeTime then
...
RegisterCommand('weather', function(source, args)
  if source == 0 then
    ...
    if isAllowedToChange(source) then
      ...
      TriggerClientEvent('chatMessage', source, '', {255, 255, 255},
        '^8Error: ^1You do not have access to that command.')
